  ...
  if(nrow(rec) >= 1) {
    ...
    for (i in unique(rec[["Path"]])) {
      ...
      if(length(paths[[1]]) > 1) {
        ...
        if(length(paths[[1]]) > 2 ) {
          for(p in paths[[1]][2:(length(paths[[1]])-1)]) {
            if(paste0("mp^", trimws(p)) %in% json$groups$mp$nodes) {
              prec_link <- link
              prec_link$source <- paste0("mp^", trimws(p))
              prec_link$target <- paste0("it^", trimws(paths[[1]][length(paths[[1]])]))
              prec_link$type <- "Pathway"
              prec_link$title <- i
              prec_link$id <- i
              
              json$links[[link_count]] <- prec_link
              link_count <- link_count + 1
            }
          } 
        } 
